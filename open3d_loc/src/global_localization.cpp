...
static bool IsFiniteTransformMsg(const geometry_msgs::msg::TransformStamped &transform)
{
    ...
    const double norm_sq = rotation.x * rotation.x + rotation.y * rotation.y +
                           rotation.z * rotation.z + rotation.w * rotation.w;
    return std::isfinite(norm_sq) && norm_sq > 1e-12;
}

static bool IsFinitePoseMsg(const geometry_msgs::msg::Pose &pose)
{
    ...
    const double norm_sq = orientation.x * orientation.x + orientation.y * orientation.y +
...
void GloabalLocalization::CallbackBaselink2Odom(const nav_msgs::msg::Odometry::SharedPtr baselink2odom)
{
    ...
    if (loc_initialized_)
    {
	    ...
	    if (!IsFinitePoseMsg(motionlink2map.pose.pose))
	    {
	        RCLCPP_WARN_THROTTLE(
	            this->get_logger(), *this->get_clock(), 2000,
	            "Skip invalid motion_link localization output before publish: "
	            "pos=(%.3f, %.3f, %.3f), quat=(%.6f, %.6f, %.6f, %.6f)",
	            motionlink2map.pose.pose.position.x,
	            motionlink2map.pose.pose.position.y,
	            motionlink2map.pose.pose.position.z,
	            motionlink2map.pose.pose.orientation.x,
	            motionlink2map.pose.pose.orientation.y,
	            motionlink2map.pose.pose.orientation.z,
	            motionlink2map.pose.pose.orientation.w);
	        return;
	    }
	    ...
	    if (!IsFiniteTransformMsg(transform) || !IsFinitePoseMsg(motionlink2map.pose.pose))
	    {
	        RCLCPP_WARN_THROTTLE(
	            this->get_logger(), *this->get_clock(), 2000,
	            "Skip invalid motion_link localization output: "
	            "pos=(%.3f, %.3f, %.3f), quat=(%.6f, %.6f, %.6f, %.6f)",
	            motionlink2map.pose.pose.position.x,
	            motionlink2map.pose.pose.position.y,
	            motionlink2map.pose.pose.position.z,
...
void GloabalLocalization::Localization()
{
    ...
    {
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 10000, "Waiting for Odometry_loc...");
    ...
    while (rclcpp::ok())
    {
        ...
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 10000, "Waiting for cloud_registered_1...");
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
